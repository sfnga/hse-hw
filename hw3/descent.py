...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        return 2 / len(y) * (np.dot(x.T, self.predict(x) - y))
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        return 2 / self.batch_size * (np.dot(
            x[indices].T,
            self.predict(x[indices]) - y[indices]))
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        nu = self.lr()
        diff = nu / (np.sqrt(v_hat) + self.eps) * m_hat
...
class BaseDescentReg(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        l2_gradient: np.ndarray = np.zeros_like(
            x.shape[1])  # TODO: replace with L2 gradient calculation
...
def get_descent(descent_config: dict) -> BaseDescent:
    ...
    descent_mapping: Dict[str, Type[BaseDescent]] = {
        ...
        if not regularized else VanillaGradientDescentReg,
        'stochastic':
        StochasticDescent if not regularized else StochasticDescentReg,
